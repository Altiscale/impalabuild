...
impala_version_file="$curr_dir/version.info"
GIT_HASH=""
...
cloudera.base-branch=$BRANCH_NAME\ncloudera.build-branch=cdh5-1.4_5.1.3\ncloudera.pkg.version=$IMPALA_VERSION\
\ncloudera.pkg.release=1.4+cdh5.1.3+0\ncloudera.pkg.name=impala\ncloudera.cdh.release=NA\naltiscale.hive.version=$ALTISCALE_HIVE_VERSION\ncloudera.build.time=$(date -u +%Y.%m.%d-%H:%M:%S%Z)" > "$cdh_property_file"

echo -e "#Autogenerated build properties\nVERSION=$IMPALA_VERSION\nGIT_HASH=${GIT_HASH}\nBUILD_TIME=$(date -u +%Y.%m.%d-%H:%M:%S%Z)" > "$impala_version_file"
