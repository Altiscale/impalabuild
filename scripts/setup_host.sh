...
MOCK_UID=34567
MOCK_USER=makerpm
impala_spec="$local_dir/impala.spec"
...
# Install all dependent package for build process later on for this Host
...
libevent-devel automake libtool flex bison gcc-c++ openssl-devel \
make cmake doxygen.x86_64 glib-devel python-devel bzip2-devel svn libevent-devel cyrus-sasl-devel

yum -y install fedora-packager rpmdevtools

# If this breaks, point to a better repo
yum -y --enablerepo=verticloud-test install s3cmd
...
if [ -d "/opt/apache-ant" -a -d "/opt/apache-maven" ] ; then
  echo "ok - detected existing maven and ant installed"
elif [ -d "$M2_HOME" -a -d "$ANT_HOME" ] ; then
  echo "ok - detected existing maven and ant installed"
elif [ -d "$MAVEN_HOME" -a -d "$ANT_HOME" ] ; then
  echo "ok - detected existing maven and ant installed"
else
